...
class Creator:
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONUP:
                    ...
                    if sldr.getYLoc() * const.scaleFactor <= mY <= (
                            sldr.getYLoc() + sldr.getMax() * 12) * const.scaleFactor:
                        sldr.setValue(
                            int(floor(((mY - (sldr.getYLoc()) * const.scaleFactor) / 12) / const.scaleFactor)))
                    else:
                        sldr.sliding = False
                        return
    ...
    def calculateStats(self):
        ...
        self.str = self.bStr + ((((self.sliders[0].getMax() - self.sliders[1].getValue()) * 10) / 2) / 5) + 2 * (
                    1 - self.sliders[2].getValue())
        ...
        if step == 'stats':
            ...
            for s in self.sliders:
                if s.sliding:
                    ...
                    self.displayField.blit(self.images[0], (s.getXLoc(), s.getYLoc() + (s.getValue() * 10)))
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    (mX, mY) = pygame.mouse.get_pos()
                    for s in self.sliders:
                        if s.getXLoc() * const.scaleFactor <= mX < (s.getXLoc() + 24) * const.scaleFactor and (
                                s.getYLoc() + (s.getValue() * 10)) * const.scaleFactor <= mY < (
                                s.getYLoc() + (s.getValue() * 10) + 24) * const.scaleFactor:
                            self.slide(s, screen)
                    # done
                    if int(floor(33 * const.scaleFactor)) <= mX < int(ceil(82 * const.scaleFactor)) and int(
                            floor(237 * const.scaleFactor)) <= mY < int(ceil(266 * const.scaleFactor)):
                        self.displayField, r = load_image.load_image(os.path.join('MENU', "createBox2.png"))
