...
from ..MAP import cavegen
from ..MAP import generalmap  # , mapgen  #, mazegen
from ..MAP import mazegen
...
class World:
    ...
    def __init__(self, context, myWorldBall=None, loadWorld=None):
        if context == 'editor':
            ...
            if loadWorld is not None:
                ...
                self.installWorldBall(context, myWorldBall, None, )

    def loadWorld(self, context):
        filename = os.getcwd() + '/MAP/WORLDS/MainWorld'
        try:
            loadedWorld = gzip.GzipFile(filename, 'rb', 1)
            ball = cPickle.load(loadedWorld)
            loadedWorld.close()
            self.installWorldBall(ball, context)
        except IOError as message:
            print('Cannot load world:', filename, message)
            return
    ...
    def generateMap(self, dimension, level, type):
        ...
        # if type == 'dungeon':
        #     MG = cavegen.Generator(dimension, level)
        #     MG.generateMap(20)
    ...
    def changeMap(self, dir, loc):
        if dir == 0:  # north
            if self.currentMap.neighbors[0] != '':
                if self.getMapByName(self.currentMap.neighbors[0]).getTileFG(
                        loc[0], self.getMapByName(self.currentMap.neighbors[0]).getDIM() - 1) in range(const.BRICK1):
            ...
            if self.currentMap.neighbors[3] != '':
                if self.getMapByName(self.currentMap.neighbors[3]).getTileFG(
                        self.getMapByName(self.currentMap.neighbors[3]).getDIM() - 1,
